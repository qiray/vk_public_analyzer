...
def dateposts(db):
    ...
    months = sorted([datetime.datetime.fromtimestamp(x[0]).strftime('%m') for x in posts]) #months
    # dates = sorted([datetime.datetime.fromtimestamp(x[0]).strftime('%Y.%m.%d') for x in posts]) #dates
    # print(dates)

    years_names = sorted(list(set(years)))
    get_dateposts('posts_years.png', years, years_names, None)

    # month_names = [calendar.month_name[i + 1] for i in range(12)] #TODO: fix
    # get_dateposts('posts_months.png', months, sorted(list(set(months))), month_names)
